 ...
 */
export const getAccessURLUploadsStore = () => {
  ...
  const uploadsBucketName = readEnv(
 ...
 */
export const getAccessURLStaticStore = () => {
  const shared = getSharedS3Env();
  const uploadsEndpoint = readEnv(() => process.env.UPLOADS_S3_BUCKET_ENDPOINT);
  const uploadsBucketName = readEnv(
    () => process.env.UPLOADS_S3_FILES_BUCKET_NAME
  ...
  const staticBucketName = readEnv(
 ...
 */
export const getAccessURLS3FileStore = () => getAccessURLUploadsStore();
